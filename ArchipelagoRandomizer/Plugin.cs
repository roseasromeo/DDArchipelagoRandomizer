...
#nullable enable
	...
	private void Awake()
	{
		...
		try
		{
			...
			if (BepInEx.Bootstrap.Chainloader.PluginInfos.ContainsKey("deathsdoor.returntospawn"))
			{
				DisableItemChangerSpawnOverride();
			}
	...
	private static void DisableItemChangerSpawnOverride()
	{
		// If ReturnToSpawn is installed, disable ItemChanger's override of the DoorOverrider
		Instance.harmony.Unpatch(AccessTools.Method(typeof(DoorOverrider), nameof(DoorOverrider.activate)), HarmonyPatchType.Prefix, "deathsdoor.itemchanger");
	}
